...
addEventListener($filterTogglers, "click", function () {
  $filterBar.classList.toggle("active");
  $overlay.classList.toggle("active");
  const bodyOverflow = document.body.style.overflow;
  document.body.style.overlay = bodyOverflow === "hidden" ? "visible" : "hidden";
});
...
const renderRecipe = data => {
  data.hits.map((item, index) => {
    ...
    $card.innerHTML = `
        ...
        <img src="${image}"
            width="195" height="195" loading="lazy"
...
fetchData(queries || defaultQueries, data => {
  ...
  if (data.hits.length) {
    ...
    $loadMore.innerHTML = `
    <p class="info-text">No Recipe Found</p> 
    `;
  }
});
...
window.addEventListener("scroll", async e => {
  if ($loadMore.getBoundingClientRect().top < window.innerHeight &&
    !requestedBefore && nextPageUrl) {

    $loadMore.innerHTML = $skeletonCard.repeat(Math.round(($loadMore.clientWidth /
      (CONTAINER_MAX_WIDTH)) * CONTAINER_MAX_CARD));
  ...
  if (!nextPageUrl) $loadMore.innerHTML = `
      <p class="info-text">No More Recipes</p> 
  `;
});